...
public class AgentMove : MonoBehaviour
{
    ...
    public IEnumerator MoveToPositionWithRemoveScaffolding(Vector3 destination, Vector3Int scaffoldingPos, float approachDistance = 0.1f)
    {
        var distance = Vector3.Distance(transform.position, destination);
        if (distance < approachDistance)
        {
        ...
        while (agent.remainingDistance > approachDistance)
        {
            if(Vector3.Distance(agent.transform.position, scaffoldingPos) > 1.5f)
            {
                break;
            }
