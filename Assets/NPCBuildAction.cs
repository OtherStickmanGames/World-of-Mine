...
namespace Ururu
{
    public class NPCBuildAction : MonoBehaviour
    {
        ...
        private IEnumerator BuildLadderForBlock(Vector3 destination)
        {
            ...
            WorldGenerator.Inst.SetBlockAndUpdateChunck(ladderBase, scaffoldingBlockID);
            ...
            // Перемещаем NPC к точке, где будет строиться лестница
            yield return StartCoroutine(MoveToPosition(ladderBase, false));

            // Начинаем строить лестницу снизу вверх
            float startY = Mathf.Min(transform.position.y, destination.y); // Берем минимальный уровень (на случай, если нужно строить и вниз)
            float endY = Mathf.Max(transform.position.y, destination.y);  // Берем максимальный уровень (куда нужно добраться)
            ...
            while (currentY <= endY)
            {
                Vector3 ladderBlockPos = new Vector3(ladderBase.x, currentY, ladderBase.z);
                ...
                if (WorldGenerator.Inst.GetBlockID(ladderBlockPos) == 0)
                {
                    ...
                    Debug.Log("Установлен блок лестницы на " + ladderBlockPos);
                    placedAnyBlocks = true;
                }

                // Перемещаем NPC к следующему шагу лестницы
                Vector3 nextStepPos = new Vector3(ladderBase.x, currentY + 1f, ladderBase.z);
                yield return StartCoroutine(MoveToPosition(nextStepPos, false));
                yield return new WaitForSeconds(0.1f); // Задержка между шагами
        ...
        public Edge GetClosestEdge(Vector3 buildingPosition, Vector3 size, Vector3 destination)
        {
            ...
            Vector3 minBounds = buildingPosition - halfSize;
            Vector3 maxBounds = buildingPosition + halfSize;
            ...
            float distanceToFront = Mathf.Abs(destination.z - minBounds.z);
            float distanceToBack = Mathf.Abs(destination.z - maxBounds.z);
